"""Pure LLA-absorb attention math for one layer x loop on vLLM's paged cache (torch + ``hla.lla`` only).

The LLA baseline (arXiv 2607.15456, reproduced in ``hla/lla``) stores one training-free PCA latent per
...
requests; ``tests/test_lla_serving.py`` checks it on CPU against the ``hla.lla`` attention algebra.
"""
from __future__ import annotations
from dataclasses import dataclass
import torch
from hla.lla.attention import rope_pair_index
from . import s6_ops
...
class LLAReaders:
    ...
    ``dec[H, 2*T*D, R]`` / ``mu[H, 2*T*D]`` come from ``hla.lla.fit`` (per-head PCA, nested ranks: any
