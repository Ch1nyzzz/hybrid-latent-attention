...
Environment (read once at interpreter start):
  S6_VLLM_OURO       off (default) | 1 / alias: load the adapter file under the in-tree module name
                     ...
                     `hla.vllm_latent.ouro_latent:OuroForCausalLM` (needs an adapter with absolute imports).
  S6_VLLM_OURO_FILE  adapter path for alias mode (default: hla/vllm_latent/ouro_latent.py found on sys.path).
...
ADAPTER = "hla.vllm_latent.ouro_latent"
ADAPTER_REL = os.path.join("hla", "vllm_latent", "ouro_latent.py")
...
def plan(env: dict, search_path=()) -> tuple[str, str | None]:
    ...
    if not path:
        raise ValueError("S6_VLLM_OURO=alias needs S6_VLLM_OURO_FILE or hla on sys.path")
    return "alias", path
