...
def normalize_cfg(cfg, state):
    """Drop the gated-residual keys that 0922-0924 checkpoints record as disabled.

    Trainer and vLLM must derive the same cfg because weight sync compares cfg dicts.
    """
    cfg = dict(cfg)
    if cfg.pop('gated', False) or any('.inter_s.' in k for k in state):
        raise ValueError('Gated-residual S6 checkpoints are no longer supported')
    cfg.pop('bottleneck', None); cfg.pop('legacy', None)
    return cfg
