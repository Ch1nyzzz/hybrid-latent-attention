#!/usr/bin/env bash
# trisol OPD job entry: bash hla/trisol/run_opd.sh {fkl|rkl}. Mounts: model-0 = Stage1 training.pt,
...
mkdir -p /work/hla /work/s6_deps /work/pinned-verl /trisol/output
python - <<'PY'
...
PY
tar --no-same-owner -xzf "$ASSET/s6-code.tar.gz" -C /work/hla
...
export PYTHONPATH=/work/pinned-verl:/work/s6_deps:/work/hla
cd /work/hla
python hla/trisol/restore_s6_corpus.py
python - <<'PREEMPT'
...
exec python -m hla.trisol.run_decode_math_intervals \
