...
(`hla/vllm_latent/s6_sitecustomize.py` copied to /work/s6shim, PYTHONPATH + S6_VLLM_OURO + S6_VLLM_OURO_FILE), so
...
from hla.vllm_latent.serving_config import compare_runtime_check, kv_capacity, over_capacity_message, parse_engine_log, ramp_steps
...
DATA = "hla/matheval/data/math500.jsonl"
...
GPU_TESTS = "hla/tests/test_s6_ops_gpu.py"
...
def case_env(kind: str, gpu: int, root: str, shim: str, base_env: dict, cache_root: str) -> dict:
    ...
    if kind in ADAPTERS and shim != "copy":
        env.update(S6_VLLM_OURO=shim, S6_VLLM_OURO_FILE=os.path.join(root, "hla", "vllm_latent", ADAPTERS[kind]))
    return env
...
def qual_compare_argv(method: str, ref: Path, out: Path, log: Path, compile_config: str = "", python: str = sys.executable) -> list[str]:
    argv = [python, "-m", "hla.vllm_latent.compare", "--model", MODEL, *_method_args(method), "--out", str(out), "--ref", str(ref),
...
def tp_compare_argv(method: str, prompt: int, concurrency: int, out: Path, log: Path, python: str = sys.executable, gen_tokens: int = GEN_TOKENS,
    ...
    return [python, "-m", "hla.vllm_latent.compare", "--model", MODEL, *_method_args(method, lla_codec), "--out", str(out),
...
def hf_reference_argv(method: str, out: Path, python: str = sys.executable) -> list[str]:
    return [python, "-m", "hla.latent.hf_reference", "--model-path", MODEL, *(["--base"] if method == "base" else ["--student", STUDENT]),
...
def qualify_argv(method: str, compare: Path, out: Path, python: str = sys.executable) -> list[str]:
    return [python, "-m", "hla.latent.qualify_vllm_math", "--model", MODEL, *(["--base"] if method == "base" else ["--student", STUDENT]),
...
def noise_floor(base_lp, s6_lp, k: int = LOGPROBS_K) -> dict:
    ...
    import torch
    from hla.latent.logprob_metrics import position_metrics, summarize
    positions = []
...
def main(argv=None):
    ...
    p.add_argument("--root", type=Path, default=Path("/work/hla")); p.add_argument("--out", type=Path, default=Path("/trisol/output/vllm-fused"))
