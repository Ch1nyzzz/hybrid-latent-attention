...
import subprocess
import sys
from hla.trisol.math500_intervals import evaluate,inference_env,FDO
...
def prepare_resume_export(source,out):
    ...
    import torch
    from hla.latent.training_common import SEMANTICS
...
def run(train_root=None,root=None,out=None):
    train_root=Path(train_root or '/work/hla');root=Path(root or Path(__file__).resolve().parents[2])
    ...
    def train(target,resume=None,qualify=False):
        ...
        args=['bash',str(train_root/'hla/trisol/run_stage1_recipe.sh'),*common,'--stop-after',str(target)]
    ...
    if resume_source:
        ...
        verifier = root/'hla/trisol/verify_s6_stage1_qualification.py'
        if not verifier.exists():
            verifier = train_root/'hla/trisol/verify_s6_stage1_qualification.py'
    ...
    def padded(step):
        ...
        subprocess.run([sys.executable,'-m','hla.latent.pad_serving_rank',str(out/f'student-{step}.pt'),str(path)],check=True)
        return path
    ...
    def hf(args,log):
        with (qual/log).open('w') as f:
            try:
                ...
                raise
    hf(['-m','hla.latent.hf_reference','--model-path',model,'--student',str(student),'--data',data,
    ...
    with (qual/'compare.log').open('w') as f:
        try:
            subprocess.run([sys.executable,'-m','hla.vllm_latent.compare','--model',model,'--student',str(student),
            ...
            raise
    hf(['-m','hla.latent.qualify_vllm_math','--model',model,'--student',str(student),
