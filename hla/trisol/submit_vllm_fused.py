...
python hla/trisol/submit_vllm_fused.py --dry-run          # build the bundle, print the argv, submit nothing
python hla/trisol/submit_vllm_fused.py                    # upload + submit (run by the main agent, not workflows)
python hla/trisol/submit_vllm_fused.py --code-version 5   # reuse an uploaded code version
python hla/trisol/submit_vllm_fused.py --suite-args "--mode peak --skip-qualification"   # extra suite flags
python hla/trisol/submit_vllm_fused.py --lla-codec-asset loop-lla-codec-0917:1 --name loop-lla-vllm-peak-0917 \
...
The bootstrap (bash -lc) verifies the archive sha256, extracts it to /work/hla, installs the transformers 4.56
wheels into /work/hf-deps, stages the sitecustomize shim, greps the vLLM files that are not in the local source cache
(recorded in the job log), then execs hla/trisol/run_vllm_fused_suite.py with --ouro-shim plus any --suite-args
...
FORCED = ("hla/matheval/data/math500.jsonl",)
...
def bundle_members(root: Path) -> list[str]:
    """Repo-relative paths of hla/ minus caches, results, artifacts, data, runs and weights; math500.jsonl forced in."""
    members = set()
    for path in (root / "hla").rglob("*"):
...
mkdir -p /work/hla /work/hf-deps /work/s6shim
python - <<'INSTALL'
...
cp /work/hla/hla/vllm_latent/s6_sitecustomize.py /work/s6shim/sitecustomize.py
...
export PYTHONPATH=/work/hf-deps:/work/hla
cd /work/hla
exec python hla/trisol/run_vllm_fused_suite.py --ouro-shim {shim}{extra}
"""
