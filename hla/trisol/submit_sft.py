...
Usage:
  python -m hla.trisol.submit_sft --upload-only
  python -m hla.trisol.submit_sft --dry-run --arm both
  python -m hla.trisol.submit_sft --arm base
  python -m hla.trisol.submit_sft --arm latent
  python -m hla.trisol.submit_sft --arm latent --history-backend gemm --history-precision tf32
"""
...
# Latent multipass history attention kernel (see hla/latent/history_gemm.py)
...
PY
    torchrun --standalone --nproc-per-node="$GPUS" -m hla.latent.train_sft \\
        --arm latent \\
...
def main(argv=None):
    ...
    p.add_argument("--history-backend", choices=("triton", "gemm", "reference"), default="triton",
                   help="Latent multipass history attention kernel (gemm = hla/latent/history_gemm.py)")
