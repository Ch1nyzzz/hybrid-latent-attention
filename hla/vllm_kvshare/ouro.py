...
from vllm.model_executor.models.ouro_kvshare_attention import OuroPrefixAttention, shared_decode_attention
...
class OuroAttention(nn.Module):
    def __init__(
        ...
        self.share_decode_kv = bool(getattr(config, "share_decode_kv", False))
        self.rswa_window = int(getattr(config, "rswa_window", None) or 32)
        self.share_recent = int(getattr(config, "share_recent", 16))
        self.total_ut_steps = total_ut_steps
        self.block_size = cache_config.block_size if cache_config is not None else 16
        if self.share_decode_kv:
            assert self.rswa_window >= self.share_recent + self.block_size, "rswa_window must cover recent + one block"
        ...
        for ut_step in range(total_ut_steps):
            ...
            extra = ({"layer_idx": unique_layer_idx, "dual_chunk_attention_config": dual_chunk_attention_config}
                     if dual_chunk_attention_config else {})
            if self.share_decode_kv and ut_step < total_ut_steps - 1:
                attn = OuroPrefixAttention(self.num_heads, self.head_dim, self.scaling, num_kv_heads=self.num_kv_heads,
                                           cache_config=cache_config, quant_config=quant_config, attn_type=attn_type,
                                           prefix=f"{unique_prefix}.attn", rswa_window=self.rswa_window, **extra)
            else:
                attn = Attention(self.num_heads, self.head_dim, self.scaling, num_kv_heads=self.num_kv_heads,
                                 cache_config=cache_config, quant_config=quant_config, attn_type=attn_type,
                                 prefix=f"{unique_prefix}.attn", **extra)
    ...
    def forward(
        ...
        if self.share_decode_kv and current_ut < self.total_ut_steps - 1:
            attn_output = shared_decode_attention(q, k, v, self.attn[current_ut], self.attn[-1], self.num_heads,
                                                  self.num_kv_heads, self.head_dim, self.scaling, self.block_size,
                                                  self.share_recent)
        else:
