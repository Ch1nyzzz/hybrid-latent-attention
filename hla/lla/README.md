...
```bash
python -m hla.lla.prepare_tokens --model-path $OURO --blocks 512 --output fit_tokens.npy
python -m hla.lla.prepare_tokens --model-path $OURO --blocks 32 --skip 20000 --output dev_tokens.npy
python -m hla.lla.fit     --model-path $OURO --tokens fit_tokens.npy --ranks 32,64,128,256,512 --output out/
python -m hla.lla.quality --model-path $OURO --tokens dev_tokens.npy --codecs out/lla_r*.pt --output out/quality.json
python -m hla.lla.bench   --model-path $OURO --codecs out/lla_r128.pt --contexts 1024,4096,16384 --output out/bench.json
python -m hla.lla.saturate --model-path $OURO --codecs out/lla_r512.pt out/lla_r128.pt --contexts 4096,16384,65536 --output out/sat.json
```

Tests (no weights needed): `python -m pytest hla/tests/test_lla.py`.
