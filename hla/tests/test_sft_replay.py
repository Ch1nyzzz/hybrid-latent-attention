...
from hla.latent.sft_replay import (
...
def test_checkpoint_roundtrip_latent(tmp_path):
    from hla.latent.training_common import atomic_checkpoint, restore_checkpoint, make_full_parameter_optimizer
...
def test_checkpoint_roundtrip_base(tmp_path):
    from hla.latent.training_common import atomic_checkpoint, restore_checkpoint, trainable_parameters
...
def test_sft_dataset(tmp_path):
    from hla.latent.sft_replay import SFTDataset
    import json
...
def test_train_sft_main_both_arms(tmp_path, monkeypatch):
    ...
    from hla.latent import train_sft
    ...
    with patch("hla.vendor.modeling_ouro.OuroForCausalLM.from_pretrained", return_value=model):
        train_sft.main([
            "--arm", "base",
    ...
    with patch("hla.latent.train_sft.load_student_backbone", return_value=model):
        train_sft.main([
            "--arm", "latent",
            "--model-path", "/dummy/path",
