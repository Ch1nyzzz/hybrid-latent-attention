...
def test_interval_driver_and_trainer_forward_backend():
    ...
    from hla.latent import train_decode
    ...
    args = train_decode.parse(argv[argv.index('hla.latent.train_decode') + 1:])
...
def test_linear_warmup_schedule_and_resume_state():
    from hla.latent.train_decode import warmup_factor, parse
    from hla.trisol.run_decode_math_intervals import training_args
    ...
    assert parse(argv[argv.index('hla.latent.train_decode') + 1:]).warmup_steps == 10
