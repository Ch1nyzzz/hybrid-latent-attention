...
import torch
import pytest
from hla.tests.test_khop_replay import tiny_double, teacher_pass
from hla.latent.decode_training import Trajectory
from hla.latent.history_snapshot import collect_snapshot
from hla.latent.khop_replay import replay_batch_khop
from hla.latent.fkl import memory_bounded_fkl
from hla.trisol.run_decode_math_intervals import training_args
from hla.latent.train_decode import parse
...
def test_fkl_rollout_cache_matches_collect_and_checks_drift(response):
    ...
    with patch('hla.latent.khop_replay.load_rollout_snapshot',return_value=snap):
...
def test_fkl_trainer_resume(tmp_path):
    from hla.tests.test_s6_direct_decode import make_inputs, reference_worker
    ...
    with patch('hla.latent.teacher.load_teacher',side_effect=lambda *a,**k:deepcopy(model)), \
