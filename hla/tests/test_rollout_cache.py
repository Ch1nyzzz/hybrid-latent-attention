...
from hla.vllm_latent.cache_export import RolloutCacheExporter
from hla.latent.decode_training import Trajectory
from hla.latent.history_snapshot import load_rollout_snapshot
...
def test_rollout_replay_never_collects(tmp_path):
    from hla.tests.test_khop_replay import tiny_double
    ...
    with patch('hla.latent.khop_replay.collect_snapshot', side_effect=AssertionError('serial collect')):
...
def test_external_internal_request_id_mapping():
    from hla.vllm_latent.rollout_worker import generate_with_request_ids
    def assign(req):
