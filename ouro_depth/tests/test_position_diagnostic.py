...
from ouro_depth.latent.position_diagnostic import (equivariant_projection, project_reader,
    capture_layer, attention_probe, probe_metrics, probe_loss, fixed_rollout,
    fit_and_probe, rollout_probe, VARIANTS)
from ouro_depth.latent.register import apply_rope, rope_latent
...
def test_two_shard_actual_fit_probe_and_logits(tmp_path):
    ...
    with patch('ouro_depth.latent.teacher.Teacher',side_effect=new_teacher):
    ...
    with patch('ouro_depth.latent.vendor_model.load_teacher',side_effect=lambda *a,**k:deepcopy(model)):
    ...
    from ouro_depth.latent.summarize_position_diagnostic import summarize
