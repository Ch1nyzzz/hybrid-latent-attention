...
def test_new_corpus_initialization_and_resume(tmp_path):
 ...
 out=tmp_path/'out'
 common=['--mode','opd','--opd-divergence','fkl','--replay-strategy','khop','--replay-backend','serving','--model-path','tiny','--data-dir',str(data),'--steps','2','--global-batch-size','2','--tbptt','2','--max-prompt-length','8','--max-response-length','5','--save-every','1','--eval-every','2','--eval-records','2','--output-dir',str(out),'--expected-stage1-manifest',old]
