...
from ouro_depth.latent.diagnostic_suite import (
    capture_layer, run_slice_probe, run_free_oracle, run_prompt_credit_probe, run_diagnostics
)
from ouro_depth.latent.summarize_diagnostic import (
...
def test_two_shard_runner_and_summary(tmp_path):
    ...
    with patch('ouro_depth.latent.teacher.Teacher', side_effect=lambda *a, **k: Teacher.wrap(deepcopy(model))):
        for rank in range(2):
