...
the codec is ``ouro_depth.lla.fit`` output (per-head PCA, nested ranks, bf16 or fp32). No student weights: the
frozen body is loaded by vLLM, the readers are buffers built from the codec. Throughput baseline only: the LLA
absorb path has no HF qualification gate here (its accuracy is measured by ``ouro_depth.lla.quality``).
"""
import json
...
from ouro_depth.vllm_latent import lla_layer, ouro_latent, s6_layer, s6_ops
from ouro_depth.vllm_latent.geometry import rope_theta
