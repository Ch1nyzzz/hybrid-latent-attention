...
case "${MODE:?}" in
  compare)
    ...
    run_logged "$OUT/compare.log" python ouro_depth/vllm_latent/compare.py --model "$MODEL" --student "$STUDENT" --out "$OUT/compare" --ref "$REF" ${COMPARE_ARGS:-} ;;
...
PY
    ...
    for combo in ${KERNELTEST_ARGS:-TRITON_ATTN:512 TRITON_ATTN:256}; do be=${combo%%:*}; rank=${combo##*:}
      ...
      run_logged "$OUT/kt_${be}_$rank.log" timeout 1500 python ouro_depth/vllm_latent/compare.py --model "$MODEL" --student "$OUT/rand_student_$rank.pt" --out "$OUT/kt_${be}_$rank" --backend "$be" --throughput 4 --tp-tokens 32 --max-model-len 2048 || failed=1
  ...
  tp)  # throughput sweep over batch sizes: TP_SEQS="32 128", TP_ARGS may hold --base / --backend X
    for n in ${TP_SEQS:-32 128}; do echo "=== tp seqs=$n"
      run_logged "$OUT/tp_$n.log" python ouro_depth/vllm_latent/compare.py --model "$MODEL" --student "$STUDENT" --out "$OUT/tp_$n" --throughput "$n" --tp-tokens ${TP_TOKENS:-512} --max-model-len ${TP_MAXLEN:-4096} ${TP_ARGS:-}
    done ;;
...
PY
  ...
  dbg)  # synchronous CUDA launches so the failing kernel shows in the traceback; DBG_ARGS as for compare
    ...
    test -n "$REF" || { echo "REFERENCE_MISSING"; exit 1; }
    run_logged "$OUT/dbg.log" env VLLM_ENABLE_V1_MULTIPROCESSING=0 CUDA_LAUNCH_BLOCKING=1 python ouro_depth/vllm_latent/compare.py --model "$MODEL" --student "$STUDENT" --out "$OUT/dbg" --ref "$REF" ${DBG_ARGS:-} ;;
