"""Build code package with new GatedResidual writer, upload model asset, and submit Stage 1 job to Trisol."""
...
JOB_NAME = "loop-s6-stage1-gated-res-k512v512-r1-512-1000step-0923b"
OUTPUT_MODEL = "loop-s6-stage1-gated-res-k512v512-r1-512-1000step-0923b"

DESCRIPTION = (
    "Stage1 Gated Residual writer 1000 steps: low-rank bottleneck b=64 reading uncompressed hidden states, "
    "K512/V512 with loop-1 rank1 expanded to 512 (K512/V512/R1-512). "
...
def build_package(root: Path, pkg_dir: Path) -> dict:
    ...
    # 1. Package recipe-code.tar.gz from local ouro_depth (ensuring new GatedResidual is included!)
    recipe_tar_path = pkg_dir / "recipe-code.tar.gz"
    recipe_sha = package_ouro_depth(root, recipe_tar_path)

    # 2. Package eval-code.tar.gz from local ouro_depth
    eval_tar_path = pkg_dir / "eval-code.tar.gz"
    eval_sha = package_ouro_depth(root, eval_tar_path)
    ...
    provenance = {
        "change": "Stage1 Gated Residual writer b=64 raw-hidden input, K512/V512 with rank1=512",
        "geometry": {"rank_k": 512, "rank_v": 512, "rank1": 512, "bottleneck": 64},
...
def upload_code_asset(pkg_dir: Path, version_name: str) -> int:
    ...
    cmd = [
        ...
        "--description", "Stage 1 Gated Residual writer b=64 raw-hidden, K512/V512/R1-512",
        "-y",
        "-o", "json"
    ]
...
def submit_job(version_code: int) -> dict:
    ...
    # Archival
    out_dir = Path("/Users/erv1n/loop_scale/artifacts/s6-gated-res-k512v512-r1-512-1000step-20260923b")
    ...
    results_receipt = Path("/Users/erv1n/loop_scale/results/latent/s6-stage1-gated-res-k512v512-r1-512-1000step-20260923b-launch.json")
    results_receipt.parent.mkdir(parents=True, exist_ok=True)
    receipt_data = {
        "task": "S6 Stage 1 Gated Residual writer (b=64, raw-hidden) K512/V512 Rank1=512 1000-step training with MATH-500 intervals",
...
def main():
    ...
    pkg_dir = Path("/Users/erv1n/loop_scale/artifacts/s6-gated-res-k512v512-r1-512-1000step-20260923b/package")
    ...
    version_name = f"gated-res-b64-{int(datetime.datetime.now().timestamp())}"
