...
def build_package(root: Path, pkg_dir: Path) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        shutil.copy2(root / "ouro_depth/trisol/verify_s6_stage1_qualification.py",
                     tmp_path / "ouro_depth/trisol/verify_s6_stage1_qualification.py")
        ...
        with gzip.GzipFile(filename="", mode="wb", fileobj=recipe_buf, mtime=0) as gz, tarfile.open(fileobj=gz, mode="w") as tar:
            for p in sorted((tmp_path / "ouro_depth").rglob("*")):
