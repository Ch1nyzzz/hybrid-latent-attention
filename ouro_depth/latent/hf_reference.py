"""HF-side reference for the vLLM latent-cache implementation: greedy decode (self key via the final reader set) on a
...
def main():
    ...
    dec = LatentDecoder(model, student, max_len=4096, self_final=True)
    ...
    json.dump({"student_cfg": ck["cfg"], "student": args.student, "max_new": args.max_new, "prompts": out}, open(Path(args.output) / "hf_reference.json", "w"))
